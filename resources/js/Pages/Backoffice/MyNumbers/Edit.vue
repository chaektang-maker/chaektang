...
const form = useForm({
    ...
    running_numbers: Array.isArray(props.number.running_numbers)
        ? props.number.running_numbers.join(',')
        : (props.number.running_numbers || ''),
});

const submit = () => {
    form.put(route('backoffice.my-numbers.update', props.number.id));
};
</script>

<template>
    <Head title="แก้ไขเลขของสำนักฉัน" />

    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between gap-4">
                <h2 class="text-xl font-semibold leading-tight text-gray-800">แก้ไขเลขของสำนักฉัน</h2>
